...
<div class="d-flex flex-wrap justify-content-evenly blog-container">
  ...
  @endguest
    <blog-card id="{{$post->id}}" title="{{$post->title}}" image="{{ asset('images/' . $post->image)}}" categories="{{$post->category}}" blogHTML="{{ $post->blogHTML }}" date="{{date_format(date_create($post->created_at), "M j, Y")}}"></blog-card>
